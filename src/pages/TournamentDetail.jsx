...
export default function TournamentDetail() {
  ...
  const winnerIds =
    tournament?.matchType === "solo"
        ? [
                tournament?.results?.first?.gameId,
                        tournament?.results?.second?.gameId,
                                tournament?.results?.third?.gameId
                                      ]
                                          : [
...
{activeTab === 'results' && (
  <div className="mt-4 animate-fade-in">
    ...
    <Card className="bg-dark-400 p-4">
...
<div className="mt-2 space-y-2">{tournament.results.first.teamMembers
  .filter(player =>
    !tournament.results?.excludedPlayers?.includes(player.gameId || player.odeuId)
  )
  .map((player, i) => {
...
<div className="mt-2 space-y-2">{tournament.results.second.teamMembers
  .filter(player =>
    !tournament.results?.excludedPlayers?.includes(
      player.gameId || player.odeuId
    )
...
<div className="mt-2 space-y-2">{tournament.results.third.teamMembers.filter(player =>
    !tournament.results?.excludedPlayers?.includes(player.gameId || player.odeuId))
  .map((player, i) => {
...
(
tournament.results.third?.teamMembers?.filter(
player =>
!tournament.results?.excludedPlayers?.includes(
player.gameId || player.odeuId
)
...
Kill Prize: ₹{(tournament.results.third.kills || 0) * (tournament.perKillPrize || 0)}
  ...
  {tournament?.perKillPrize > 0 &&
    tournament?.participantDetails?.filter((player) => {
      ...
      <Card className="bg-dark-400 p-4 mt-4">
        <div className="flex items-center gap-3 mb-4">
